...
Knob.prototype = {
  ...
  _handleWheelEvents: function (e) {
    e.preventDefault();
    let deltaX = -e.detail || e.wheelDeltaX;
    let deltaY = -e.detail || e.wheelDeltaY;
    let val = deltaX > 0 || deltaY > 0 ? 1 : deltaX < 0 || deltaY < 0 ? -1 : 0;
    this.changed(val);
  },
  ...
  _updateWhileMoving: function (event) {
    ...
    let x = this.centerX - e.pageX;
    let y = this.centerY - e.pageY;
    let deg =
    ...
    let step = (this.settings.max - this.min) / range;
...
//       let end = this.options.steps + (this.options.anglerange == 360 ? 0 : 1);
//       this.ticks = [];
//       let Shape = this.options.type;
//       for (let i = 0; i < end; i++) {
//         let rect = new Shape(
...
//   let dialStep = (this.options.max - min) / this.options.steps;
//   let end = this.options.steps + (this.options.anglerange == 360 ? 0 : 1);
//   this.dials = [];
//   if (!this.options.labels) {
//     for (let i = 0; i < end; i++) {
//       let text = new Ui.El.Text(
...
//       let label = this.options.labels[i];
//       let text = new Ui.El.Text(
//         label,
//         this.width / 2 - 2.5,
//         this.height / 2 - this.options.radius,
...
//   let startAngle = this.options.angleoffset;
//   let outerRadius = this.options.outerRadius || this.options.width / 2;
//   let innerRadius =
//     this.options.innerRadius || this.options.width / 2 - this.options.arcWidth;
//   let startAngleDegree = (Math.PI * startAngle) / 180;
//   let endAngleDegree = (Math.PI * (startAngle + angle)) / 180;
//   let center = this.options.width / 2;
//
//   let p1 = pointOnCircle(outerRadius, endAngleDegree);
//   let p2 = pointOnCircle(outerRadius, startAngleDegree);
//   let p3 = pointOnCircle(innerRadius, startAngleDegree);
//   let p4 = pointOnCircle(innerRadius, endAngleDegree);
//
